...
@Component
class LightStateProjector : EventProjector {

    override fun supports(eventType: String): Boolean =
        eventType == EventType.LIGHT_STATE_CHANGED.wireName

    override fun project(
        event: DomainEvent<*>
    ): ProjectionCommand {

        val payload = event.payload as? LightStateChanged
            ?: throw IllegalArgumentException(
                "Expected LightStateChanged payload, got " +
        ...
        return ProjectionCommand(
            ...
            attributes = mapOf(
                ...
                "lightOn" to ProjectionValue.BooleanValue(payload.on),
                "brightnessPercent" to ProjectionValue.NumberValue(
